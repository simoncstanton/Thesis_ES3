...
class Obs_utility:
    ...
    def set_basepath(self, obs_data):
        ...
        print(obs_data["obs_invocation"]["home"])
    ...
    def meta_access_exp_result_set(self, obs_data, subjob_id):
        ...
        obs_data["obs_exp"]["exp_episodes"] = exp_data["world_properties"]["episodes"]
        obs_data["obs_exp"]["exp_timesteps"] = exp_data["world_properties"]["timesteps"]
    ...
    def extract_exp_result_set_summary(self, obs_data, subjob_id):
        ...
        path = os.path.join(os.sep.join(self.hpc_config["journal"]['exp_journal_path']), obs_data["obs_exp"]["exp_parent_id"], obs_data["obs_exp"]["exp_parent_id"]+ "_" + str(subjob_id))
    ...
    def set_obs_data_start(self, obs_data):
        ...
        obs_data["obs_exp"]["exp_journal_path"] = list(self.hpc_config["journal"]['exp_journal_path']) + [obs_data["obs_exp"]["exp_parent_id"]]
        

    def set_obs_data_start_compile_cputime(self, obs_data):
    ...
    def get_process_time_ns(self, obs_data, sj):
        #print(obs_data)
        #path = os.path.join(os.sep.join(self.hpc_config["journal"]['journal_path']), obs_data["obs_exp"]["exp_parent_id"], obs_data["obs_exp"]["exp_parent_id"] + "_" + str(subjob_id))
        path = os.path.join(os.sep.join(self.hpc_config["journal"]["exp_journal_path"]), obs_data["obs_exp"]["exp_parent_id"], obs_data["obs_exp"]["exp_result_set"][0], self.hpc_config["journal"]['journal_sj_summary_path'])
    ...
    def write_obs_journal(self, obs_data):
        path = os.path.join(os.sep.join(self.hpc_config["journal"]["obs_journal_path"]), obs_data["obs_exp"]["exp_parent_id"])
        file = obs_data["journal_output_filename"]
    ...
    '''
    def fetch_all_subjobs_result_set_data_tournament(self, obs_data, obs_data_summary, leaf_dir, tournament_trial_count):
        # enter here the first layer of compression - HPC pbs_scripts puts *all* data from subjob into a single tar.gz
        # whereas loclahost is run from CLI, so no 'subjob' compression
        if obs_data_summary["obs_exp"]["exp_localhost"]:
            result_set_data = self.fetch_all_uncompressed_subjobs_tournament(obs_data, obs_data_summary, leaf_dir, tournament_trial_count)
            
        else:
            result_set_data = self.fetch_all_compressed_subjobs_tournament(obs_data, obs_data_summary, leaf_dir, tournament_trial_count)
        
        return result_set_data
    
    def fetch_all_compressed_subjobs_tournament(self, obs_data, obs_data_summary, leaf_dir, tournament_trial_count):
        result_set_data = []
        ...
        for sj in obs_data_summary["obs_exp"]["exp_subjobs_list"]:
            ...
            #print("file: ", file)
            #if sj == 8:
            result_set_data.append(self.access_sj_gz_tournament(path, file, leaf_dir, obs_data_summary["obs_exp"]["exp_compressed_writes"], obs_data_summary["obs_exp"]["exp_episodes"]))
            #result_set_data.append(self.access_sj_gz_tournament(path, file, leaf_dir, obs_data_summary["obs_exp"]["exp_compressed_writes"], obs_data_summary["obs_exp"]["exp_episodes"], tournament_trial_count))  
        
        return result_set_data
  
    def fetch_all_uncompressed_subjobs_tournament(self, obs_data, obs_data_summary, leaf_dir, tournament_trial_count):
        result_set_data = []
        
        for sj in obs_data_summary["obs_exp"]["exp_subjobs_list"]:
            sj = int(sj)
            for i in range(0, tournament_trial_count):
                #print(i)
                path = os.path.join(obs_data["obs_invocation"]["basepath"], os.sep.join(obs_data_summary["obs_exp"]["exp_data_path"]), obs_data_summary["obs_exp"]["exp_result_set"][sj], str(i))
                #print(sj, path)
                result_set_data.append(self.access_sj_set_tournament(path, leaf_dir, obs_data_summary["obs_exp"]["exp_compressed_writes"]))
    ...
    def access_sj_gz_tournament(self, path, file, leaf_dir, compressed_writes, episodes):
        '''
            extract experiment result_set_data matching a specific leaf_dir pattern from 
                '.tar.gz' -> '.tar' -> ['.csv'|'.csv.gz'] -> dict(filename:list)
                    ...
                    - if compressed writes:
                        ...
                        - iterate over the sorted list:
                            - each entry is a csv.gz file in the tar.gz file (which is effectively a tar file now)
                                ...
                                - use a list comprehension to read each value from the csv object _as an int_ and *append* to a temporary list, trial_array
    ...
    def access_sj_set_tournament(self, path, leaf_dir, compressed_writes):
        '''
            we know exact path and the subjob is uncompressed
                but leaf directories may contain compressed files (compressed_writes == true)
            ...
            ** to natsort the entries, different approach here - using OrderedDict - than for access_sj_gz(),
                as there is no comparator defined for nt.entry
    ...
    ''' 
        RECOG funtions
            ...
            recog obs functions
                more duplication here - [TODO] re-organise util class (13062023)

            access_sj_set_recog etc

            

    '''
    def recog_set_obs_data_start(self, obs_data):
        obs_data["eo_id"] = obs_data["obs_exp"]["exp_parent_id"].split('_',1)[0] + "_" + obs_data["obs_id"] 
        obs_data["journal_output_filename"] = self.hpc_config["obs"]["data_file_prefix"] + obs_data["obs_exp"]["exp_parent_id"] + self.hpc_config["analysis"]["recog"]["recog"] + self.hpc_config["journal"]["journal_entry_suffix"] + self.hpc_config["journal"]["journal_entry_extension"]
        obs_data["obs_exp"]["obs_data_filename_prefix"] = self.hpc_config["obs"]["data_file_prefix"] + obs_data["obs_exp"]["exp_parent_id"]

    def make_recog_job_paths(self, obs_data, obs_data_summary):
        ...
        path = os.path.join(obs_data["obs_invocation"]["basepath"], os.sep.join(self.hpc_config["paths"]["observations"]), obs_data_summary["obs_exp"]["exp_type"], obs_data["obs_exp"]["exp_parent_id"], self.hpc_config["obs"]["leaf_view"], self.hpc_config["analysis"]["recog"]["leaf_data"])
        self.make_job_path(path)
        
        path = os.path.join(obs_data["obs_invocation"]["basepath"], os.sep.join(self.hpc_config["paths"]["observations"]), obs_data_summary["obs_exp"]["exp_type"], obs_data["obs_exp"]["exp_parent_id"], self.hpc_config["obs"]["leaf_data"], self.hpc_config["analysis"]["recog"]["leaf_data"])
        self.make_job_path(path)

    def access_sj_set_recog(self, path, leaf_dir, compressed_writes):
        '''
            we know exact path and the subjob is uncompressed
                but leaf directories may contain compressed files (compressed_writes == true)
    ...
    #def fetch_all_uncompressed_subjobs_recog(self, obs_data, obs_data_summary, leaf_dir):
    #    result_set_data = []
    #    
    #    for sj in obs_data_summary["obs_exp"]["exp_subjobs_list"]:
    #        sj = int(sj)
    #        path = os.path.join(obs_data["obs_invocation"]["basepath"], os.sep.join(obs_data_summary["obs_exp"]["exp_data_path"]), obs_data_summary["obs_exp"]["exp_result_set"][sj])
    #        result_set_data.append(self.access_sj_set_recog(path, leaf_dir, obs_data_summary["obs_exp"]["exp_compressed_writes"]))
    #
    #    return result_set_data


    def fetch_all_subjobs_result_set_data_recog(self, obs_data, obs_data_summary, leaf_dir, file_tag):
        # enter here the first layer of compression - HPC pbs_scripts puts *all* data from subjob into a single tar.gz
        # whereas loclahost is run from CLI, so no 'subjob' compression
        if obs_data_summary["obs_exp"]["exp_localhost"]:
            result_set_data = self.fetch_all_uncompressed_subjobs_recog(obs_data, obs_data_summary, leaf_dir, file_tag)
        else:
            result_set_data = self.fetch_all_compressed_subjobs_recog(obs_data, obs_data_summary, leaf_dir, file_tag)
        
        return result_set_data

    def fetch_all_compressed_subjobs_recog(self, obs_data, obs_data_summary, leaf_dir, file_tag):
        result_set_data = []
        ...
        for sj in obs_data_summary["obs_exp"]["exp_subjobs_list"]:
            ...
            result_set_data.append(self.access_sj_gz_recog(path, file, leaf_dir, obs_data_summary["obs_exp"]["exp_compressed_writes"], file_tag, obs_data_summary["obs_exp"]["exp_episodes"]))  
        
        return result_set_data


    def fetch_all_uncompressed_subjobs_recog(self, obs_data, obs_data_summary, leaf_dir, file_tag):
        result_set_data = []
        
        for sj in obs_data_summary["obs_exp"]["exp_subjobs_list"]:
            sj = int(sj)
            path = os.path.join(obs_data["obs_invocation"]["basepath"], os.sep.join(obs_data_summary["obs_exp"]["exp_data_path"]), obs_data_summary["obs_exp"]["exp_result_set"][sj])
            result_set_data.append(self.access_sj_set_recog(path, leaf_dir, obs_data_summary["obs_exp"]["exp_compressed_writes"], file_tag))
        
        return result_set_data



    def access_sj_gz_recog(self, path, file, leaf_dir, compressed_writes, file_tag, episodes):
        '''
            extract experiment result_set_data matching a specific leaf_dir pattern from 
                '.tar.gz' -> '.tar' -> ['.csv'|'.csv.gz'] -> dict(filename:list)
        '''
        ...
        if compressed_writes:
            ...
            with tarfile.open(fileobj = io.BytesIO(result_set_tar), mode = 'r') as t:
                ...
                for file_in_tar in t.getnames():
                    ...
                    if file_in_tar.lower().endswith(self.hpc_config["exp"]["recog_uncompressed_extension"]) and leaf_dir in file_in_tar:
                        if file_tag in file_in_tar: 
                            found_csv.append(file_in_tar)
                ...
                for file in found_csv:
                    
                    z = 1
                    single_file = io.TextIOWrapper(t.extractfile(file), encoding="utf-8")
                    filename_f = str(file).split('/')[2]

                    # get filename
                    filename_t = found_csv[i].split('/')[2]
                    
                    # get timestep                   
                    recog_data = json.load(single_file)
                    
                    if file_tag == "gamelock" or file_tag == "gamelock_reward" or file_tag == "gamelock_pref":
                        
                        if recog_data:
                            recog_dict = recog_data[0]
                        else:
                            recog_dict = {'0':  [str({''}) , []]}

                        # get match set
                        for t, j in recog_dict.items():
                            result_set_data[filename_t] = [[t],[j]]
                
                        i = i + 1

                    elif file_tag == "interlock" or file_tag == "interlock_reward" or file_tag == "interlock_pref":
                        ...
                        if z > 1000:
                            ...
                            x = z
                        result_set_data[filename_f] = [[x],[l[0]]]
    ...
    def access_sj_set_recog(self, path, leaf_dir, compressed_writes, file_tag):
        '''
            we know exact path and the subjob is uncompressed
                but leaf directories may contain compressed files (compressed_writes == true)
            ...
            with os.scandir(os.path.join(path, leaf_dir)) as result_set_ls:
                ...
                for entry in result_set_ls:
                    ...
                    if entry.is_file():
                        ...
                        if file_tag in entry.name:
                            
                            with open(entry, newline='') as file:
                                
                                recog_data = json.load(file)
                            
                                if file_tag == "gamelock_reward" or file_tag == "gamelock_pref":
                                    
                                    if recog_data:
                                        recog_dict = dict(recog_data[0])
                                    
                                        for e, j in recog_dict.items():
                                            result_set_data[entry.name] = [[e],[j]]
                                    else:
                                        result_set_data[entry.name] = [[0], []]
                                
                                elif file_tag == "interlock_reward" or file_tag == "interlock_pref":
                                    ...
                                    if z > 1000:
                                        ...
                                        x = z
                                    result_set_data[entry.name] = [[x],[l[0]]]             

            result_set_data = natsorted(result_set_data.items(), key=lambda t: t[0])
            
        #return OrderedDict(natsorted(result_set_data.items(), key=lambda t: t[0]))
        return OrderedDict(result_set_data)
    




    def recog_assemble_gamelock_df(self, result_set_data, obs_data, obs_data_summary, file_tag):
        
        import model.rgs.rgs_complete as rgs_complete
        import pandas as pd

        rgs = rgs_complete.Rgs_complete(False)

        i = 1
        id = []             # Row ID                auto 
        game_ID = []        # Game Model            map from sj_id
        game_found = []     # Identified Game Model extract from rs_item
        episode = []        # Episode ID            extract from sj_data_gamelock key
        lock_count = []     # Gamelock Count        iterate over all eps, count empties, subtract from ep_count
        average_t = []      # Average Time to Lock  iterate over all eps, sum non-empties, take mean
        min_t = []          # Lowest Time to Lock   iterate over all eps, get lowest ttl (time_to_lock)
        max_t = []          # Highest Time to Lock  iterate over all eps, get highest ttl (time_to_lock)
        t_set = []          # TTL Set               iterate over all eps, append t
        ...
        for rs in result_set_data:
            ...
            for rs_key, rs_item in rs.items():
                ...
                if file_tag == "gamelock_reward":
                    ep = rs_key.split("_")[7].split(".")[0]
                elif file_tag == "gamelock_pref":
                    ep = rs_key.split("_")[8].split(".")[0]
                
                g_id = rgs.map_instance_id_to_gameform_name(int(sj))

                sj_t_set.append(int(rs_item[0][0]))
                
                if int(rs_item[0][0]) != 0:
                    lc = lc + 1
                    ff = rs_item[1][0]
                    g = ff[0][2:6]
                
            
            sum_t_set = sum(sj_t_set)
            id.append(i)
            game_ID.append(g_id)
            game_found.append(g)
            episode.append(ep)
            lock_count.append(lc)
            if lc != 0:
                average_t.append(sum_t_set/lc)
            else:
                average_t.append(0)

            min_t.append(min(i for i in sj_t_set if i > 0))
            max_t.append(max(sj_t_set))
            
            t_set.append(sj_t_set)

            i = i + 1
            

        sj_count = 1
        xaxis_count = range(1, (sj_count) + 1)
        data_traces = {
            "ID": pd.Series(id, index = xaxis_count),
            "Game_ID": pd.Series(game_ID, index = xaxis_count),
            "Recog": pd.Series(game_found, index = xaxis_count),
            "Episodes": pd.Series(episode, index = xaxis_count),
            "Lock_Count": pd.Series(lock_count, index = xaxis_count),
            "Average_TTL": pd.Series(average_t, index = xaxis_count),
            "Min_TTL": pd.Series(min_t, index = xaxis_count),
            "Max_TTL": pd.Series(max_t, index = xaxis_count),
            "All_TTL": pd.Series(t_set, index = xaxis_count),
        }
        df = pd.DataFrame(data_traces)

        path2 = os.path.join(obs_data["obs_invocation"]["basepath"], os.sep.join(self.hpc_config["paths"]["observations"]), obs_data_summary["obs_exp"]["exp_type"], obs_data_summary["obs_exp"]["exp_parent_id"], "data", "recog")
        file2 = "view_" + obs_data_summary["obs_exp"]["exp_parent_id"] + "_recog_complete_" + file_tag + ".csv"
    ...
    def recog_assemble_interlock_df(self, result_set_data, obs_data, obs_data_summary, file_tag, agent_id):
        ...
        i = 1
        id = []             # Row ID                auto 
        agent = []          # Agent ID              extract from filename/rs_key
        game = []           # Game Model            map from sj_id
        episode = []        # Episode ID            extract from sj_data_gamelock key
        lock_count = []     # Gamelock Count        iterate over all eps, count empties, subtract from ep_count
        average_t = []      # Average Time to Lock  iterate over all eps, sum non-empties, take mean
        min_t = []          # Lowest Time to Lock   iterate over all eps, get lowest ttl (time_to_lock)
        ...
        for rs in result_set_data:
            ...
            for rs_key, rs_item in rs.items():
                ...
                if rs_key.split("_")[7] == agent_id:
                    
                    a = rs_key.split("_")[7]
                    sj = re.search("^exp_\d*_(\d*)_", rs_key)[1]
                    g = rgs.map_instance_id_to_gameform_name(int(sj))
                    ep = rs_key.split("_")[9].split(".")[0]
                    sj_t_set.append(int(rs_item[0][0]))
                    ...
                    if int(rs_item[0][0]) != 0:
                        ...
                        if rs_item[1][0] == candidates:
                            pass
                        else:
                            candidates = rs_item[1][0]
                            #candidate_set_interstitial.update( {int(rs_item[0][0]) :rs_item[1][0] } )
        ...
        self.make_recog_interlock_df(obs_data_summary, obs_data, file_tag, agent_id, id, game, agent, episode, lock_count, average_t, min_t, max_t, candidate_set, t_set)





    def make_recog_interlock_df(self, obs_data_summary, obs_data, file_tag, a_id, id, game, agent, episode, lock_count, average_t, min_t, max_t, candidate_set, t_set):  
        ...
        data_traces = {
            "ID": pd.Series(id, index = xaxis_count),
            "Game": pd.Series(game, index = xaxis_count),
            "Agent": pd.Series(agent, index = xaxis_count),
            "Episodes": pd.Series(episode, index = xaxis_count),
            "Lock_Count": pd.Series(lock_count, index = xaxis_count),
            "Average_TTL": pd.Series(average_t, index = xaxis_count),
            "Min_TTL": pd.Series(min_t, index = xaxis_count),
            "Max_TTL": pd.Series(max_t, index = xaxis_count),
            "CandidateSet": pd.Series(candidate_set, index = xaxis_count),
            "All_TTL": pd.Series(t_set, index = xaxis_count),
        }
        df = pd.DataFrame(data_traces)

        path2 = os.path.join(obs_data["obs_invocation"]["basepath"], os.sep.join(self.hpc_config["paths"]["observations"]), obs_data_summary["obs_exp"]["exp_type"], obs_data_summary["obs_exp"]["exp_parent_id"], "data", "recog")
        file2 = "view_" + obs_data_summary["obs_exp"]["exp_parent_id"] + "_recog_" + file_tag + "_agent_" + a_id + ".csv"
        filename = os.path.join(path2, file2)
    ...
    def retrieve_obs_data_summary(self, obs_data):
        ...
        path = os.path.join(os.sep.join(self.hpc_config["journal"]["obs_journal_path"]), parent_id)
    ...
    def ep_o_assemble_terminal_e_count_tournament(self, result_set_data, obs_data, obs_data_summary):
        ...
        for rs in result_set_data:
            ...
            for rs_key, rs_item in rs.items():
                ...
                sj = re.search("^\d*_(\d*)_", rs_key)[1]
                outcome = rs_key.split('_')[4]
    ...
    '''
        ep_r_assemble_agent_terminal_sum_tournament()
    ...
    def ep_r_assemble_agent_terminal_sum_tournament(self, result_set_data, obs_data, obs_data_summary):
    
        sj_data_terminal_sum = {  }

        for rs in result_set_data:
            for rs_key, rs_item in rs.items():

                tokens = rs_key.split('_')
                ...
                if sj not in sj_data_terminal_sum:
                    sj_data_terminal_sum[sj] = { }

                if trial_id not in sj_data_terminal_sum[sj]:
                    sj_data_terminal_sum[sj][trial_id] = { }

                if episode not in sj_data_terminal_sum[sj][trial_id]:
                    sj_data_terminal_sum[sj][trial_id][episode] = {}

                if agent not in sj_data_terminal_sum[sj][trial_id][episode]:
                    sj_data_terminal_sum[sj][trial_id][episode][agent] = sum(rs_item)
     

        for sj, trial in sj_data_terminal_sum.items():
            sj_write_data = []

            # "1" hack to bypass episode iteration. [research code! 05/06/2023]
            for t in trial:
                #print(trial[t]["1"])
                #print(sj_data_terminal_sum[sj][t]["1"])
                sj_write_data.append(sj_data_terminal_sum[sj][t]["1"])
            
            obs_data["obs_exp"]["exp_subjobs"][sj]["data_files"]["terminal_sum" + "_sj_" + sj] = self.eo_ep_r_write_obs_data(obs_data, obs_data_summary, sj_write_data, sj, self.hpc_config["analysis"]["ep_r"]["terminal_sums_tournament"] )
